...
CREATE VIEW [dbo].[Apparel_Merchandising_Stock_Department_LFL_Week_Fact_View]
AS
...
SELECT
	...
	,CASE WHEN lfl.LFL_LY = 1 THEN f.Net_OnHand_Qty ELSE 0 END AS Net_OnHand_Qty_LFL_LY
	,CASE WHEN lfl.LFL_LY = 1 THEN f.Net_OnHand_FP_Qty ELSE 0 END AS Net_OnHand_FP_Qty_LFL_LY
	,CASE WHEN lfl.LFL_LY = 1 THEN f.Net_OnHand_MD_Qty ELSE 0 END AS Net_OnHand_MD_Qty_LFL_LY

FROM
    dbo.Apparel_Store_Item_Stock_Week_Fact f
JOIN dbo.Item_Dim i
    ON i.Item_Dim_ID = f.Item_Dim_ID
JOIN dbo.Apparel_Merchandising_Department_LFL_Week_Fact lfl
    ON lfl.Store_Dim_ID = f.Store_Dim_ID
       AND lfl.Department_Dim_ID = i.Department_Dim_ID
