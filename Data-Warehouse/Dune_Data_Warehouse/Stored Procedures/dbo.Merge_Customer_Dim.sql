...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Merge_Customer_Dim]
	@Source VARCHAR(MAX),
	@Log_ID BIGINT,
	@Cache BIT,
	@Process BIT
...
INSERT INTO @return
EXEC dbo.Log_Start @Source, 'Merge_Customer_Dim', @Cache, @Process;
...
MERGE INTO dbo.Customer_Dim target
USING ( 
SELECT
	Customer_Dim_ID
	,Customer_Number
...
FROM Dune_Data_Warehouse_Staging..Customer_Dim
WHERE NewOrChangedCustomer = 'TRUE'
) source 
ON source.Customer_Dim_ID = target.Customer_Dim_ID
WHEN MATCHED THEN 
UPDATE SET  Customer_Title = source.Customer_Title                       
			...
			,Current_Customer_Flag = source.Current_Customer_Flag
...
SET @Error_Response = 'An Error has occured in stored procedure dbo.Merge_Customer_Dim: (%d)%s';
