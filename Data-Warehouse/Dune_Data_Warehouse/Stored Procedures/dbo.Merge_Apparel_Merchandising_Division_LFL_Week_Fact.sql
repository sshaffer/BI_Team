...
CREATE PROCEDURE [dbo].[Merge_Apparel_Merchandising_Division_LFL_Week_Fact]
    @Source VARCHAR(MAX)
   ,@Log_ID BIGINT
   ,@Cache BIT
   ,@Process BIT
...
INSERT  INTO @return
        EXEC dbo.Log_Start
            @Source
           ,'Merge_Apparel_Merchandising_Division_LFL_Week_Fact'
           ,@Cache
           ,@Process;
...
BEGIN TRY
    ...
    MERGE INTO dbo.Apparel_Merchandising_Division_LFL_Week_Fact target
    USING
        ( SELECT
...
BEGIN CATCH
    ...
    SET @Error_Response = 'An Error has occured in stored procedure dbo.Merge_Apparel_Merchandising_Division_LFL_Week_Fact: (%d)%s';
