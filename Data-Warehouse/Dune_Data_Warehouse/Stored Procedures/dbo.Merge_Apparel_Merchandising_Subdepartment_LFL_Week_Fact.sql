...
CREATE PROCEDURE [dbo].[Merge_Apparel_Merchandising_Subdepartment_LFL_Week_Fact]
    @Source VARCHAR(MAX)
   ,@Log_ID BIGINT
   ,@Cache BIT
   ,@Process BIT
...
INSERT  INTO @return
        EXEC dbo.Log_Start
            @Source
           ,'Merge_Apparel_Merchandising_Subdepartment_LFL_Week_Fact'
           ,@Cache
           ,@Process;
...
BEGIN TRY
    ...
    MERGE INTO dbo.Apparel_Merchandising_Subdepartment_LFL_Week_Fact target
    USING
        ( SELECT
            thisYear.Calendar_Week_Dim_Id
           ,thisYear.Store_Dim_ID
           ,thisYear.Subdepartment_Dim_ID
          ...
          LEFT JOIN ( SELECT
            ...
            ON lastYear.Subdepartment_Dim_ID = thisYear.Subdepartment_Dim_ID
          ...
          LEFT JOIN ( SELECT
            ...
            ON nextYear.Subdepartment_Dim_ID = thisYear.Subdepartment_Dim_ID
	...
	USING (
          SELECT
           ...
           ,s.Store_Dim_ID
           ,d.Subdepartment_Dim_ID
           ,LFL = 1
           ,LFL_LY = 1
          FROM
            dbo.BIS_Store_Dim s
          CROSS JOIN dbo.Subdepartment_Dim d
...
BEGIN CATCH
    ...
    SET @Error_Response = 'An Error has occured in stored procedure dbo.Merge_Apparel_Merchandising_Subdepartment_LFL_Week_Fact: (%d)%s';
