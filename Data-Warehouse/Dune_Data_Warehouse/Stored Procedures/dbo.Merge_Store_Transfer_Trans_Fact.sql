...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Merge_Store_Transfer_Trans_Fact]
	@Source VARCHAR(MAX),
	@Log_ID BIGINT,
	@Cache BIT,
	@Process BIT
...
INSERT INTO @return
EXEC dbo.Log_Start @Source, 'Merge_Store_Transfer_Trans_Fact', @Cache, @Process;
...
MERGE INTO dbo.Store_Transfer_Trans_Fact target
USING (	
	SELECT
	...
	FROM Dune_Data_Warehouse_Staging..Store_Transfer_Trans_Fact 
	GROUP BY 
		Transfer_Date_Dim_ID
	   ,From_Store_Dim_ID
	   ,Item_Dim_ID
	...
	ON source.Distribution_Number = target.Distribution_Number
	AND source.Distribution_Sequence_Number = target.Distribution_Sequence_Number
	AND source.Allocation_Number = target.Allocation_Number
	AND source.Transfer_Date_Dim_ID = target.Transfer_Date_Dim_ID
...
INSERT (
	...
	,source.Allocation_Number
	,source.Distribution_Sequence_Number
);
...
SET @Error_Response = 'An Error has occured in stored procedure dbo.Merge_Store_Transfer_Trans_Fact: (%d)%s';
