...
CREATE PROCEDURE [dbo].[Merge_OCS_Fulfillment_Transfer_Trans_Fact]
	@Source VARCHAR(MAX),
	@Log_ID BIGINT,
	@Cache BIT,
	@Process BIT
...
INSERT INTO @return
EXEC dbo.Log_Start @Source, 'Merge_OCS_Fulfillment_Transfer_Trans_Fact', @Cache, @Process;
...
MERGE INTO dbo.OCS_Fulfillment_Transfer_Trans_Fact target
USING Dune_Data_Warehouse_Staging.dbo.OCS_Fulfillment_Transfer_Trans_Fact source
...
WHEN MATCHED THEN
    UPDATE SET
			  ...
			  ,Sending_Store_Dim_Id = source.Sending_Store_Dim_Id
...
SET @Error_Response = 'An Error has occured in stored procedure dbo.Merge_OCS_Fulfillment_Transfer_Trans_Fact: (%d)%s';
