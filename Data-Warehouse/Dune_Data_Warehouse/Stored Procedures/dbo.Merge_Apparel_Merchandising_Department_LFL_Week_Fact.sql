...
CREATE PROCEDURE [dbo].[Merge_Apparel_Merchandising_Department_LFL_Week_Fact]
    @Source VARCHAR(MAX)
   ,@Log_ID BIGINT
   ,@Cache BIT
   ,@Process BIT
...
INSERT  INTO @return
        EXEC dbo.Log_Start
            @Source
           ,'Merge_Apparel_Merchandising_Department_LFL_Week_Fact'
           ,@Cache
           ,@Process;
...
BEGIN TRY
    ...
    MERGE INTO dbo.Apparel_Merchandising_Department_LFL_Week_Fact target
    USING
        ( SELECT
            thisYear.Calendar_Week_Dim_Id
           ,thisYear.Store_Dim_ID
           ,thisYear.Department_Dim_ID
          ...
          LEFT JOIN ( SELECT
            ...
            ON lastYear.Department_Dim_ID = thisYear.Department_Dim_ID
          ...
          LEFT JOIN ( SELECT
            ...
            ON nextYear.Department_Dim_ID = thisYear.Department_Dim_ID
	...
	USING (
          ...
          FROM
            dbo.BIS_Store_Dim s
          CROSS JOIN dbo.Department_Dim d
