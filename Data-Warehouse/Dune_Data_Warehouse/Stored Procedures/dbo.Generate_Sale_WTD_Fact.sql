...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[Generate_Sale_WTD_Fact]
	@Source VARCHAR(100),
	@Log_ID BIGINT,
	@Cache BIT,
	@Process BIT
...
INSERT INTO @return
EXEC dbo.Log_Start @Source, 'Generate_Sale_WTD_Fact', @Cache, @Process;
...
/* Variables */
DECLARE @Fiscal_Week_Beginning_Dim_ID BIGINT;
DECLARE @Fiscal_Week_Ending_Dim_ID BIGINT;

/* Set Variables */
SELECT 
	@Fiscal_Week_Beginning_Dim_ID = b.Calendar_Dim_ID
	,@Fiscal_Week_Ending_Dim_ID = e.Calendar_Dim_ID 
FROM Calendar_Dim cal
JOIN Calendar_Dim b 
	ON b.Calendar_Date = cal.Fiscal_Week_Beginning_Date 
JOIN Calendar_Dim e
	ON e.Calendar_Date = cal.Fiscal_Week_Ending_Date
WHERE cal.Relative_Day = 0;

/* Truncate Table */
TRUNCATE TABLE dbo.Sale_WTD_Fact;

INSERT INTO dbo.Sale_WTD_Fact (
	Item_Dim_ID
	,Store_Dim_ID
	,Channel_Dim_ID
	,Currency_Dim_ID
...
SET @Error_Response = 'An Error has occured in stored procedure dbo.Generate_Sale_WTD_Fact: (%d)%s';
