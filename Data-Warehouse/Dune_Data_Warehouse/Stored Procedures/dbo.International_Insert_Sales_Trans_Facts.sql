...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[International_Insert_Sales_Trans_Facts]
	@Source VARCHAR(MAX),
	@Log_ID BIGINT,
	@Cache BIT,
	@Process BIT
...
INSERT INTO @return
EXEC dbo.Log_Start @Source, 'International_Insert_Sales_Trans_Facts', @Cache, @Process;
...
MERGE INTO dbo.International_Sales_Trans_Fact target
USING (
	SELECT
		Sale_Date_Dim_ID
		,Time_Dim_ID
	...
	ON target.Sale_Date_Dim_ID = source.Sale_Date_Dim_ID
...
SET @Error_Response = 'An Error has occured in stored procedure dbo.International_Insert_Sales_Trans_Facts: (%d)%s';
