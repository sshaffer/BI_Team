...
CREATE PROCEDURE [dbo].[Merge_No_of_Stores_Week_Fact]
	@Source VARCHAR(MAX),
	@Log_ID BIGINT,
	@Cache BIT,
	@Process BIT
...
INSERT INTO @return
EXEC dbo.Log_Start @Source, 'Merge_No_of_Stores_Week_Fact', @Cache, @Process;
...
WITH stock AS (
	SELECT 
		...
		SUM(f.Stock_in_Hand_Qty) AS Qty
...
MERGE INTO dbo.No_of_Stores_Week_Fact target
USING (
	SELECT
...
SET @Error_Response = 'An Error has occured in stored procedure dbo.Merge_No_of_Stores_Week_Fact: (%d)%s';
